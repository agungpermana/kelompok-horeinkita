...
<div class="flex items-center gap-3">
    <div class="text-right hidden sm:block">
        ...
        <p class="text-xs text-on-surface-variant">{{ $myWarung->lokasi_rw ?? '-' }}, Kel. Sukmajaya</p>
    </div>
...
@php
    ...
    $totalTransaksi = \App\Models\transaksi_donasi::count();
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
    ...
    <div class="bg-white rounded-xl border border-outline-variant/30 shadow-sm p-5">
        <div class="flex items-start justify-between mb-3">
            <p class="text-sm text-on-surface-variant">Total Sembako</p>
    ...
    <div class="bg-white rounded-xl border border-outline-variant/30 shadow-sm p-5">
        <div class="flex items-start justify-between mb-3">
            <p class="text-sm text-on-surface-variant">Penerima Terdaftar</p>
    ...
    <div class="bg-white rounded-xl border border-outline-variant/30 shadow-sm p-5">
        <div class="flex items-start justify-between mb-3">
            <p class="text-sm text-on-surface-variant">Transaksi Masuk</p>
            <span class="material-symbols-outlined text-green-600 text-xl" data-weight="fill">receipt</span>
        </div>
        <p class="text-3xl font-bold text-on-surface">{{ $totalTransaksi }}</p>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3 p-3 rounded-lg hover:bg-surface-container-low transition-all">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-on-surface truncate">Kupon #{{ $bukti->id_kupon }}</p>
                        <p class="text-xs text-on-surface-variant">
                            Bukti #{{ $bukti->id_bukti }} &bull;
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0
                        {{ $status === 'selesai' ? 'bg-green-100 text-green-700' :
                           ($status === 'proses'  ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700') }}">
                        {{ ucfirst($status) }}
                    </span>
                </div>
                @endforeach
    ...
    <div class="bg-white rounded-xl border border-outline-variant/30 shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('katalog-paket.index') }}"
               class="px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-semibold hover:opacity-90 transition-all">
                Update Stok
            </a>
        </div>
        ...
        @if($stokMenipis->count() > 0 || $stokHabis->count() > 0)
            <table class="w-full">
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    @foreach($stokHabis->take(3) as $item)
                    <tr>
