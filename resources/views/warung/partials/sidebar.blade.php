...
<a class="flex items-center gap-3 px-4 py-3 rounded-lg text-on-surface-variant hover:bg-surface-container-low transition-all" href="{{ route('dashboard') }}">
...
<a class="flex items-center gap-3 px-4 py-3 rounded-lg {{ $active === 'katalog' ? 'bg-secondary-container text-on-secondary-container font-bold shadow-[0px_2px_4px_rgba(0,0,0,0.05)]' : 'text-on-surface-variant hover:bg-surface-container-low' }} transition-all" href="{{ route('warung.katalog.index') }}">
<span class="material-symbols-outlined" {{ $active === 'katalog' ? 'data-weight=fill' : '' }}>shopping_basket</span>
