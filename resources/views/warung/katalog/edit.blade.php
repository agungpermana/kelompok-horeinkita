...
@section('page_title', 'Edit Katalog Sembako')
...
@section('content')
            ...
            <form class="space-y-stack-xl" action="{{ route('warung.katalog.update', $paket->id_paket) }}"
                method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-stack-md">
                        <div class="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                id="nama_paket" name="nama_paket" type="text"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea
                                ...
                                id="deskripsi" name="deskripsi" rows="4" placeholder="Deskripsi isi paket (opsional)">{{ old('deskripsi', $paket->deskripsi) }}</textarea>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-gutter">
                        <div class="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                value="{{ old('harga', $paket->harga) }}" placeholder="Contoh: 50000" required />
                            @error('harga')
                                <span class="text-error font-label-sm text-label-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="flex flex-col gap-2">
                            ...
                            <input
                                class="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary transition-all font-body-sm text-on-surface placeholder:text-on-surface-variant/50"
                                id="stok" name="stok" type="number" min="0"
                                value="{{ old('stok', $paket->stok) }}" placeholder="Contoh: 10" required />
                            @error('stok')
                                <span class="text-error font-label-sm text-label-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
