<!DOCTYPE html>
<html lang="id"><head>
@include('warung.partials.head', ['pageTitle' => 'Edit Katalog Sembako'])
</head>
...
<h1 class="font-headline-md text-headline-md text-on-surface">Edit Katalog Sembako</h1>
</div>
</header>
...
<form class="space-y-stack-xl" action="{{ route('warung.katalog.update', $paket->id_paket) }}" method="POST">
@csrf
@method('PUT')
...
<input class="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary transition-all font-body-sm text-on-surface placeholder:text-on-surface-variant/50" id="stok" name="stok" type="number" min="0" value="{{ old('stok', $paket->stok) }}" placeholder="Contoh: 10" required/>
@error('stok')
<span class="text-error font-label-sm text-label-sm mt-1">{{ $message }}</span>
@enderror
</div>
